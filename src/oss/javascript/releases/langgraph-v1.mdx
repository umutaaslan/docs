...
It's designed to work hand-in-hand with [LangChain v1](/oss/releases/langchain-v1) (whose `createAgent` is built on LangGraph) so you can start high-level and drop down to granular control when needed.
...
For a complete list of changes, see the [migration guide](/oss/migrate/langgraph-v1).
...
* For information on the new `createAgent` API, see the [LangChain v1 release notes](/oss/releases/langchain-v1#createagent).
* For information on migrating from `createReactAgent` to `createAgent`, see the [LangChain v1 migration guide](/oss/migrate/langchain-v1#createagent).
...
For more information on interrupts, see the [Interrupts](/oss/langgraph/interrupts) documentation.
...
See the [migration guide](/oss/migrate/langgraph-v1#stream-encoding) for more information.
...
<CardGroup cols={3}>
    ...
    <Card title="Migration guide" icon="arrow-right-arrow-left" href="/oss/migrate/langgraph-v1" arrow>
