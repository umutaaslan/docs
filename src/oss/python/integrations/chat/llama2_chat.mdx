...
For using a Llama-2 chat model with a [LlamaCPP](/oss/integrations/providers/llamacpp) `LMM`, install the `llama-cpp-python` library using [these installation instructions](/oss/integrations/providers/llamacpp#installation-and-setup). The following example uses a quantized [llama-2-7b-chat.Q4_0.gguf](https://huggingface.co/TheBloke/Llama-2-7b-Chat-GGUF/resolve/main/llama-2-7b-chat.Q4_0.gguf) model stored locally at `~/Models/llama-2-7b-chat.Q4_0.gguf`.
