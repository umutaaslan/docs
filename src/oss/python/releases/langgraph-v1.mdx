...
It's designed to work hand-in-hand with [LangChain v1](/oss/releases/langchain-v1) (whose `create_agent` is built on LangGraph) so you can start high-level and drop down to granular control when needed.
...
* For information on the new @[`create_agent`] API, see the [LangChain v1 release notes](/oss/releases/langchain-v1#create-agent).
* For information on migrating from @[`create_react_agent`] to @[`create_agent`], see the [LangChain v1 migration guide](/oss/migrate/langchain-v1#create-agent).
